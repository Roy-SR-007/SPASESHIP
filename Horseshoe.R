...
# and the prior over beta is the Horseshoe prior.

library(monomvn)
library(mvtnorm)
library(statmod)

##### data generation for Horseshoe prior and corresponding results
...
for(data in 1:100) {
  ...
  result_hs = bhs(X, y, T = niter, beta = init_params$beta, s2 = init_params$sigma_sq, verb = 0)
  beta_post_est[1, ] = colMeans(result_hs$beta)
