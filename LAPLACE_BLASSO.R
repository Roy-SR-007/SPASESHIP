...
# and the prior over beta is the LASSO prior, Bayesian LASSO.

### Under the Laplace prior (Bayesian LASSO), as a default choice, a gamma prior
### was placed on the “LASSO parameter” lambda as given in (6) of Park and Casella (2008),
### with shape and rate parameters 2 and 0.1, respectively.

library(monomvn)
library(mvtnorm)
library(statmod)

##### data generation for Bayesian LASSO and corresponding results
...
for(data in 1:100) {
  ...
  result_laplace = blasso(X, y, T = niter, beta = init_params$beta, s2 = init_params$sigma_sq, case = "default",
                            rd = c(2, 0.1), verb = 0)
  beta_post_est[1, ] = colMeans(result_laplace$beta)
