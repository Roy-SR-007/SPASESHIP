...
# and beta is estimated using the LASSO technique. LASSO penalty.

library(SIS)
library(mvtnorm)
library(statmod)

##### data generation for LASSO penalty technique and corresponding results
...
for(data in 1:100) {
  ...
  result_LASSO = SIS(X, y, family = "gaussian", penalty = "lasso", iter.max = niter)
  beta_true_dummy[result_LASSO$ix] = as.vector(result_LASSO$coef.est)[-1]
  beta_LASSO_est = beta_true_dummy
  model_error_1[data] = crossprod(beta_true - beta_LASSO_est, 
                                Sigma_X %*% as.matrix(beta_true - beta_LASSO_est))
...
for(data in 1:100) {
  ...
  beta_true_dummy[result_LASSO$ix] = as.vector(result_LASSO$coef.est)[-1]
  beta_LASSO_est = beta_true_dummy
  model_error_2[data] = crossprod(beta_true - beta_LASSO_est, 
                                Sigma_X %*% as.matrix(beta_true - beta_LASSO_est))
...
for(data in 1:100) {
  ...
  beta_true_dummy[result_LASSO$ix] = as.vector(result_LASSO$coef.est)[-1]
  beta_LASSO_est = beta_true_dummy
  model_error_3[data] = crossprod(beta_true - beta_LASSO_est, 
                                Sigma_X %*% as.matrix(beta_true - beta_LASSO_est))
